...
export const ResultsTable: React.FC<ResultsTableProps> = ({ data, detailedData }) => {
  ...
  const handleExcelExport = () => {
    exportToExcel(data, detailedData);
  };
              ...
              <tr key={row.hsCode} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-right">
                  <div className="flex items-center justify-end space-x-2">
                    ...
                    <button
                      onClick={() => copyToClipboard(row.lineCount)}
                      className="p-1 hover:bg-gray-200 rounded transition-colors duration-150"
                      title="Copy Line Count"
                    >
                      {copiedValue === row.lineCount.toString() ? (
                        <Check className="w-3 h-3 text-green-500" />
                      ) : (
                        <Copy className="w-3 h-3 text-gray-400" />
                      )}
            ...
            <tr>
              ...
              <td className="px-6 py-4 whitespace-nowrap text-sm font-bold text-blue-900 text-right">
                <div className="flex items-center justify-end space-x-2">
                  ...
                  <button
                    onClick={() => copyToClipboard(totalLines)}
                    className="p-1 hover:bg-blue-200 rounded transition-colors duration-150"
                    title="Copy Total Lines"
                  >
                    {copiedValue === totalLines.toString() ? (
                      <Check className="w-3 h-3 text-green-500" />
                    ) : (
                      <Copy className="w-3 h-3 text-blue-400" />
                    )}
