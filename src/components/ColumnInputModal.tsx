...
export const ColumnInputModal: React.FC<ColumnInputModalProps> = ({
  ...
  const handleConfirm = () => {
    ...
    const processedMapping = {
      ...
      packingList: {
        hsCode: mapping.packingList.hsCode.map(convertColumnToIndex),
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-180px)]">
          <div className="grid md:grid-cols-2 gap-8">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                ...
                {mapping.packingList.hsCode.map((column, index) => (
                  <div key={index} className="flex items-center space-x-2">
                    <input
                      type="text"
                      value={column}
                      onChange={(e) => updateColumn('packingList', 'hsCode', index, e.target.value)}
                      placeholder="F or 6"
                      className="flex-1 px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
                    {mapping.packingList.hsCode.length > 1 && (
                      <button
                        onClick={() => removeColumn('packingList', 'hsCode', index)}
                        className="p-2 text-red-500 hover:bg-red-50 rounded-md"
                      >
                        <Minus className="w-4 h-4" />
                      </button>
                ...
                <button
                  onClick={() => addColumn('packingList', 'hsCode')}
                  className="flex items-center space-x-1 text-blue-600 hover:text-blue-700 text-sm"
                >
                  <Plus className="w-4 h-4" />
                  <span>Add Column</span>
          ...
          <div className="mt-8 p-4 bg-blue-50 rounded-lg">
            ...
            <ul className="text-xs text-blue-700 space-y-1 list-disc list-inside">
              ...
              <li>Default values are provided but you can change them based on your file structure</li>
            </ul>
          </div>
        </div>
